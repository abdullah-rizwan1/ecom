    ...
    try {
        ...
        if (!suppliers) {
            return SuccessfullResponse(
                res,
                'No suppliers in supplier list.',
                suppliers
            )
        }
        return SuccessfullResponse(res, AppMessages.SUCCESS, suppliers)
    } catch (error) {
        return ErrorResponse(
            res,
            HttpCodes.INTERNAL_SERVER_ERROR,
    ...
    try {
        ...
        if (!supplier) {
            return ErrorResponse(
                res,
                HttpCodes.BAD_REQUEST,
                AppMessages.INVALID_USERNAME_EMAIL
            )
        }

        const isUserValid = await authHelper.isValidUser(
            body.password,
            supplier.password
        )
    ...
    try {
        ...
        if (!decodedToken) {
            return ErrorResponse(
                ...
                AppMessages.INVALID_TOKEN
            )
        }

        const hashedPassword = await authHelper.encryptString(password)
        const supplier = await supplierService.updateSupplierPasswordByEmail(
            decodedToken.email,
            hashedPassword
        )
